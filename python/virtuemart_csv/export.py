...
normal_fields = ('Название объекта', 'Категория', 'Цена', 'Имя агента', 'Без комиссии', 'Изображения')
...
def pars_custom_fields(row):
	...
	for i in range(len(custom_title)):											# собираем из элементов новые данные
		...
		if custom_title[i] == 'geocustom_plg': custom_title[i] = 'Геоданные'	# переименовываем 'geocustom_plg' в 'Геоданные'
